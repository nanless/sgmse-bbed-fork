...
class FourierTimestepEmbedder(nn.Module):
    ...
    Embeds scalar timesteps into vector representations.
    """
    def __init__(self, hidden_size, embedding_type='fourier', nf=128, fourier_scale=16):
        super().__init__()
        self.mlp = nn.Sequential(
            nn.Linear(nf*2, hidden_size, bias=True),
            nn.SiLU(),
            nn.Linear(hidden_size, hidden_size, bias=True),
        )
        ...
        if embedding_type == 'fourier':
            ...
            self.fourier_embedder = layerspp.GaussianFourierProjection(
                embedding_size=nf, scale=fourier_scale
            )
        elif embedding_type == 'linear':
            self.fourier_embedder = layerspp.GaussianFourierLinearProjection(
                embedding_size=nf, scale=fourier_scale
            )

    def forward(self, t):
        t_freq = self.fourier_embedder(t)
        t_emb = self.mlp(t_freq)
        return t_emb
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        self.t_embedder = FourierTimestepEmbedder(hidden_size, embedding_type, nf, fourier_scale)
...
def test_DiT():
    model = DiT().cuda()
    x = torch.complex(torch.randn(2,256,256), torch.randn(2,256,256)).cuda()
    y = torch.complex(torch.randn(2,256,256), torch.randn(2,256,256)).cuda()
